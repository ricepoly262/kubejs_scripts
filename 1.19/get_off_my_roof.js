...
RoofCleaner.finalCheck = (event, ply) => event.server.schedule(RoofCleaner.warn_time_ms, () => { // final check to see if they are still ontop of the nether roof
    ...
    if (player.persistentData.nether_kill == false) {
        if (RoofCleaner.above(ply) && (ply.persistentData.ricepoly.nether_warn == true)) {
        ...
        } else if (!RoofCleaner.above(ply) && (ply.persistentData.ricepoly.nether_warn == true)) {
...
RoofCleaner.spawnProtection = (event, ply) => event.server.schedule(RoofCleaner.spawn_protection_ms, () => { // spawn protection
    ...
    ply.persistentData.ricepoly.nether_protected = false;
})
...
PlayerEvents.loggedIn(event => { // for spawn protections
    player = event.player;
...
ServerEvents.customCommand(event => { // commands
    if (event.player.op) {
...
ItemEvents.rightClicked(event => { // voucher protection, not usable by operators
